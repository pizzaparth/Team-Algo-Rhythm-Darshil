...
export const NodeToolbar: React.FC<NodeToolbarProps> = ({ nodeId }) => {
  ...
  return (
    <div className="flex items-center space-x-1 bg-white/95 backdrop-blur-md border border-[#E5E2DD] p-1.5 rounded-full shadow-lg z-40 text-xs animate-in fade-in zoom-in-95 duration-150">
      {/* Expand (AI) */}
      <button
        onClick={() => openModal('expand_node', { nodeId })}
        disabled={isExpanding}
        className="px-3 py-1 rounded-full bg-indigo-600 hover:bg-indigo-500 text-white font-medium flex items-center space-x-1 transition-colors shadow-sm disabled:opacity-60"
      ...
      <button
        ...
        className="px-3 py-1 rounded-full bg-[#1A1A1A] hover:bg-[#2c2c2c] text-white font-medium flex items-center space-x-1 transition-colors shadow-sm"
      ...
      <button
        ...
        className="p-1.5 text-[#1A1A1A] hover:bg-[#F3F1ED] rounded-full transition-colors"
